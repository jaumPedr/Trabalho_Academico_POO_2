...
public class Main {
	...
	public static void main(String[] args) throws IOException, FileNotFoundException, InputMismatchException {
		...
		while(!escolha.equals("0")) {
			...
			case "1":
				...
				System.out.println("Digite a resposta, sem separação de espaço, 10 letras, F ou V: ");
			...
			case "3":
				...
				if(!disciplinaExiste(diretorio, nomeDisciplina)) {
					...
					if(!existeArquivo(diretorio, caminho)) {
	...
	public static String lerDisciplina() {
		...
		while(!isDisciplinaValida(nome)) {
			System.out.println("Nome inválido! Digite o nome da disciplina (sem espaços em branco entre as palavras): ");
	...
	public static boolean existeArquivo(File diretorio, String caminhoDigitado) {
		String[] lista = diretorio.list();
		String[] listaCaminhos = new String[lista.length];
		for(int i=0; i<lista.length; i++) {
			listaCaminhos[i] = diretorio.getAbsolutePath() + "\\" + lista[i];
			if(caminhoDigitado.contains(listaCaminhos[i])) {
				return true;
			}
		}
		return false;	
	...
	public static void criarArquivo(File file, ArrayList<Aluno> lista, double media) throws IOException {		
		...
		try {
			...
			bw.write("MÉDIA" + "\t" + df.format(media)); // formata a media para duas casas decimais
			bw.newLine();
		} catch (IOException e) {
			System.out.println("Erro na criação do arquivo");
		} finally {
